...
local EZIMButtonEditor                  = {}
EZIMButtonEditor.__index                = EZIMButtonEditor
EZIMButtonEditor.editButtonPopupOpen    = false
EZIMButtonEditor.editButtonUseCursor    = false
EZIMButtonEditor.editButtonAdvanced     = false
EZIMButtonEditor.editButtonSet          = ""
EZIMButtonEditor.editButtonIndex        = 0
EZIMButtonEditor.editButtonUIChanged    = false

EZIMButtonEditor.tmpButton              = nil

EZIMButtonEditor.selectedTimerType      = 1
EZIMButtonEditor.selectedUpdateRate     = 1

EZIMButtonEditor.textEditor             = Zep.Editor.new("##EZInstanceMasterEditor")
EZIMButtonEditor.textBuffer             = EZIMButtonEditor.textEditor:CreateBuffer("[EZInstanceMaster]", "")
EZIMButtonEditor.textBuffer.syntax      = 'lua'
EZIMButtonEditor.textEditor.windowFlags = bit32.bor(Zep.WindowFlags.ShowLineNumbers, Zep.WindowFlags.ShowIndicators)

function EZIMButtonEditor:RenderEditButtonPopup()
    if not self.editButtonPopupOpen then
        picker:SetClosed()
        return
    end

    local ButtonKey = ezimSettings:GetButtonSectionKeyBySetIndex(self.editButtonSet, self.editButtonIndex)
    ...
    if self.editButtonPopupOpen and shouldDrawEditPopup then
        ...
        if self.editButtonUseCursor then
            ...
            if mq.TLO.CursorAttachment and mq.TLO.CursorAttachment.Type() then
                ...
                EZIMButtonEditor.textBuffer:SetText(self.tmpButton.Cmd or "")

                for index, type in ipairs(ezimSettings.Constants.TimerTypes) do
...
function EZIMButtonEditor:SaveButton(ButtonKey)
    -- make sure the button label isn't nil/empty/spaces
    if self.tmpButton.Label ~= nil and self.tmpButton.Label:gsub("%s+", ""):len() > 0 then
        self.tmpButton.Cmd = EZIMButtonEditor.textBuffer:GetText()

        ezimSettings:GetSettings().Sets[self.editButtonSet][self.editButtonIndex] =
            ButtonKey                                                                      -- add the button key for this button set index
        ezimSettings:GetSettings().Buttons[ButtonKey] = btnUtils.shallowcopy(self.tmpButton) -- store the tmp button into the settings table
        ezimSettings:GetSettings().Buttons[ButtonKey].Unassigned = nil                       -- clear the unassigned flag

        ezimSettings:SaveSettings(true)
        EZIMButtonEditor.textBuffer:ClearFlags(Zep.BufferFlags.Dirty)
...
function EZIMButtonEditor:CloseEditPopup()
    picker:SetClosed()
...
function EZIMButtonEditor:OpenEditPopup(Set, Index)
    ...
    EZIMButtonEditor.textBuffer:SetText(button.Cmd or "")
    ...
    if not button.Unassigned and button.TimerType and button.TimerType:len() > 0 then
        ...
        for index, type in ipairs(ezimSettings.Constants.UpdateRates) do
...
function EZIMButtonEditor:CreateButtonFromCursor(Set, Index)
    self.editButtonUseCursor = true
    self:OpenEditPopup(Set, Index)
end

function EZIMButtonEditor:RenderButtonEditUI(renderButton, enableShare, enableEdit)
    -- Share Buttton
    if enableShare then
        if ImGui.Button(Icons.MD_SHARE) then
            ezimButtonHandlers.ExportButtonToClipBoard(renderButton)
        end
    ...
    if self.editButtonAdvanced then
        ...
        self.selectedUpdateRate, _ = ImGui.Combo("Update Rate", self.selectedUpdateRate,
            function(idx) return ezimSettings.Constants.UpdateRates[idx].Display end,
            #ezimSettings.Constants.UpdateRates)
        renderButton.UpdateRate = ezimSettings.Constants.UpdateRates[self.selectedUpdateRate].Value
        self.editButtonUIChanged = self.editButtonUIChanged or textChanged
    end
    ...
    EZIMButtonEditor.textEditor:Render("##EZInstanceMasterEditor", ImVec2(ImGui.GetWindowWidth() * 0.98, editHeight))
    --ImGui.PopFont()
    self.editButtonUIChanged = self.editButtonUIChanged or EZIMButtonEditor.textBuffer:HasFlag(Zep.BufferFlags.Dirty)
end

function EZIMButtonEditor:RenderTimerPanel(renderButton)
    self.selectedTimerType, _ = ImGui.Combo("Timer Type", self.selectedTimerType, ezimSettings.Constants.TimerTypes)

    renderButton.TimerType = ezimSettings.Constants.TimerTypes[self.selectedTimerType]

    if ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Custom Lua" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Seconds Timer" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Item" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Spell Gem" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "AA" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Ability" then
    ...
    elseif ezimSettings.Constants.TimerTypes[self.selectedTimerType] == "Disc" then
...
function EZIMButtonEditor:RenderIconPicker(renderButton)
    if renderButton.Icon then
        ...
        if ezimButtonHandlers.Render(renderButton, 20, false, 1, false) then
            picker:SetOpen()
        end
        ImGui.PopID()
