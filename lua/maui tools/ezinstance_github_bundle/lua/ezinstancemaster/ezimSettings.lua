...
local EZIMActors       = rawget(_G, 'EZIMActors') or require('actors')

local settings_base    = mq.configDir .. '/EZInstanceMaster'
local settings_path    = settings_base .. '.lua'


local ezimSettings                 = {}
ezimSettings.__index               = ezimSettings
ezimSettings.settings              = {}
ezimSettings.CharConfig            = string.format("%s_%s", mq.TLO.EverQuest.Server(), mq.TLO.Me.DisplayName())
ezimSettings.Constants             = {}

ezimSettings.Globals               = {}
ezimSettings.Globals.Version       = 7
ezimSettings.Globals.CustomThemes  = {}

ezimSettings.Constants.TimerTypes  = {
    "Seconds Timer",
    "Item",
    "Spell Gem",
    "AA",
...
function ezimSettings.new()
    local newSettings      = setmetatable({}, ezimSettings)
    newSettings.CharConfig = string.format("%s_%s", mq.TLO.EverQuest.Server(), mq.TLO.Me.DisplayName())


    local config, err = loadfile(mq.configDir .. '/EZ_Instance_Master_Theme.lua')
    if not err and config then
        ezimSettings.Globals.CustomThemes = config()
    end

    return newSettings
end

function ezimSettings:SaveSettings(doBroadcast)
    ...
    if not self.settings.LastBackup or os.time() - self.settings.LastBackup > 3600 * 24 then
        ...
        mq.pickle(mq.configDir .. "/EZInstanceMaster-Backups/EZInstanceMaster-backup-" .. os.date("%m-%d-%y-%H-%M-%S") .. ".lua",
            self.settings)
    end
...
function ezimSettings:NeedUpgrade()
    return (self.settings.Version or 0) < ezimSettings.Globals.Version
end

function ezimSettings:GetSettings()
    return self.settings
end

function ezimSettings:GetSetting(settingKey)
    -- main setting
...
function ezimSettings:GetCharacterWindow(windowId)
    return self.settings.Characters[self.CharConfig].Windows[windowId]
end

function ezimSettings:GetCharacterWindowSets(windowId)
...
function ezimSettings:GetCharConfig()
    return self.settings.Characters[self.CharConfig]
end

function ezimSettings:GetButtonSectionKeyBySetIndex(Set, Index)
...
function ezimSettings:GetNextWindowId()
    return #self:GetCharConfig().Windows + 1
end

function ezimSettings:GenerateButtonKey()
    local i = 1
    while (true) do
...
function ezimSettings:ImportButtonAndSave(button, save)
    local key = self:GenerateButtonKey()
    self.settings.Buttons[key] = button
    btnUtils.Output("\agImported Button: \at%s\ag as \at%s", ezimButtonHandlers.ResolveButtonLabel(button, true) or "<No Label>", key)
    if save then
...
---@param Index number
---@return table
function ezimSettings:GetButtonBySetIndex(Set, Index)
...
function ezimSettings:ImportSetAndSave(sharableSet, windowId)
    -- is setname unqiue?
...
function ezimSettings:ConvertToLatestConfigVersion()
    self:LoadSettings()
    ...
    if not self.settings.Version then
        ...
        if needsSave then
            ...
            mq.pickle(mq.configDir .. "/EZInstanceMaster-v3-" .. os.date("%m-%d-%y-%H-%M-%S") .. ".lua", self.settings)
    ...
    if (self.settings.Version or 0) < 5 then
        mq.pickle(mq.configDir .. "/EZInstanceMaster-v4-" .. os.date("%m-%d-%y-%H-%M-%S") .. ".lua", self.settings)
    ...
    if (self.settings.Version or 0) < 6 then
        mq.pickle(mq.configDir .. "/EZInstanceMaster-v5-" .. os.date("%m-%d-%y-%H-%M-%S") .. ".lua", self.settings)
        needsSave = true
        newSettings = self.settings
    ...
    if (self.settings.Version or 0) < 7 then
        mq.pickle(mq.configDir .. "/EZInstanceMaster-v6-" .. os.date("%m-%d-%y-%H-%M-%S") .. ".lua", self.settings)
        needsSave = true
        newSettings = self.settings
        ...
        if needsSave then
            self.settings = newSettings
            self:SaveSettings(true)
            btnUtils.Output("\atUpgraded to \amv%d\at!", ezimSettings.Globals.Version)
        end
    end
end

function ezimSettings:InvalidateButtonCache()
    for _, button in pairs(self.settings.Buttons) do
        button.CachedLabel = nil
    end
end

function ezimSettings:LoadSettings()
    local config, err = loadfile(settings_path)

    if err or not config then
        local old_settings_path = settings_path:gsub(".lua$", ".ini")
        printf("\ayUnable to load global settings file(%s), creating a new one from ini(%s) file!",
        ...
        if btnUtils.file_exists(old_settings_path) then
            ...
            printf("\ayUnable to load settings file(%s), creating a new config!", old_settings_path)
            self.settings = {
                Version = ezimSettings.Globals.Version,
                Sets = {
