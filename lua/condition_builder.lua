...
local BLOCKED_LUA_PATTERNS = {
    { pattern='%f[%a]function%f[%A]',      label='function' },
    { pattern='%f[%a]while%f[%A]',         label='while' },
    { pattern='%f[%a]repeat%f[%A]',        label='repeat' },
    { pattern='%f[%a]until%f[%A]',         label='until' },
    { pattern='%f[%a]for%f[%A]',           label='for' },
    { pattern='%f[%a]do%f[%A]',            label='do' },
    { pattern='%f[%a]end%f[%A]',           label='end' },
    { pattern='%f[%a]require%f[%A]',       label='require' },
    { pattern='%f[%a]loadstring%f[%A]',    label='loadstring' },
    { pattern='%f[%a]load%f[%A]',          label='load' },
    { pattern='%f[%a]dofile%f[%A]',        label='dofile' },
    { pattern='%f[%a]setfenv%f[%A]',       label='setfenv' },
    { pattern='%f[%a]getfenv%f[%A]',       label='getfenv' },
    { pattern='%f[%a]collectgarbage%f[%A]',label='collectgarbage' },
    { pattern='%f[%a]rawget%f[%A]',        label='rawget' },
    { pattern='%f[%a]rawset%f[%A]',        label='rawset' },
    { pattern='%f[%a]setmetatable%f[%A]',  label='setmetatable' },
    { pattern='%f[%a]getmetatable%f[%A]',  label='getmetatable' },
    { pattern='%f[%a]_G%f[%A]',            label='_G' },
    { pattern='%f[%a]_ENV%f[%A]',          label='_ENV' },
    { pattern='%f[%a]os%s*%.',             label='os.*' },
    { pattern='%f[%a]io%s*%.',             label='io.*' },
    { pattern='%f[%a]debug%s*%.',          label='debug.*' },
    { pattern='%f[%a]package%s*%.',        label='package.*' },
    { pattern='%f[%a]coroutine%s*%.',      label='coroutine.*' },
}

local function scrubCodeForScan(code)
...
local function findUnsafeLuaToken(code)
    local scrubbed = scrubCodeForScan(code)
    for _,rule in ipairs(BLOCKED_LUA_PATTERNS) do
        if scrubbed:find(rule.pattern) then
            return rule.label
        end
    end
    return nil
...
local function compileSandboxedExpression(expr)
    local chunk = 'return ('..expr..')'
    if loadstring then
        local fn, err = loadstring(chunk, 'ConditionBuilderEval')
        if not fn then return nil, err end
        if setfenv then setfenv(fn, SAFE_EVAL_ENV) end
        return fn, nil
    end
    return load(chunk, 'ConditionBuilderEval', 't', SAFE_EVAL_ENV)
end

local function isSafeIniPath(path)
    local p = trim(path or '')
    if p == '' then return false, 'INI path is empty.' end
    if p:find('[\r\n]') then return false, 'INI path contains invalid control characters.' end
    if p:find('%.%.') then return false, 'INI path cannot contain "..".' end
    if not p:lower():match('%.ini$') then return false, 'INI path must end with .ini.' end
    return true
end

local function isSafeHookCommand(action)
    local cmd = trim(action or '')
    if cmd == '' then return false, 'Hook action is empty.' end
    if cmd:find('[\r\n]') then return false, 'Hook action cannot contain newlines.' end
    if not cmd:match('^/') then return false, 'Hook action must start with /.' end
    return true
end
...
local function validateLua(code)
    ...
    local unsafeToken = findUnsafeLuaToken(r)
    ...
    if unsafeToken then table.insert(errs,'Unsafe Lua token blocked: '..unsafeToken) end
...
local function drawBuilder()
    ...
    if ImGui.Button((Icons.FA_PLUS_CIRCLE or '')..' Add to Condition',170,0) then
        if pb.preview~='' then
    ...
    -- ── Action buttons ──
    if ImGui.Button((Icons.FA_CHECK_CIRCLE or '')..' Validate',    130,0) then bldRunValidate() end ImGui.SameLine()
    if ImGui.Button((Icons.FA_CODE or '')..' Wrap fn',      104,0) then bld.block=buildLuaBlock(bld.input,'function') end ImGui.SameLine()
    if ImGui.Button((Icons.FA_CODE or '')..' Wrap if',      96,0) then bld.block=buildLuaBlock(bld.input,'if') end ImGui.SameLine()
    if ImGui.Button((Icons.FA_TERMINAL or '')..' MQ2 Macro',    118,0) then bld.block=buildLuaBlock(bld.input,'macro') end ImGui.SameLine()
    if ImGui.Button((Icons.FA_FILES_O or '')..' Copy Cond',    112,0) then ImGui.SetClipboardText(bld.input) bld.flash=90 end ImGui.SameLine()
    if ImGui.Button((Icons.FA_CLIPBOARD or Icons.FA_FILES_O or '')..' Copy Block',   118,0) then ImGui.SetClipboardText(bld.block) bld.flash=90 end ImGui.SameLine()
    if ImGui.Button((Icons.FA_TRASH_O or Icons.FA_TIMES or '')..' Clear',        88,0) then
    ...
    ImGui.SameLine()
    if ImGui.Button((Icons.FA_FLOPPY_O or '')..' Save##savebtn',84,0) then
    ...
    if ImGui.Button((Icons.FA_FOLDER_OPEN_O or Icons.FA_FOLDER_OPEN or '')..' Saves##showsv',90,0) then bld.showSaves=not bld.showSaves end
    ...
    if ImGui.Button((Icons.FA_DOWNLOAD or '')..' Apply to MAUI INI',170,0) then
        if trim(bld.input)=='' or trim(bld.mauiFile)=='' then
            bld.saveMsg='Need condition + INI path'
        else
            local okPath, pathErr = isSafeIniPath(bld.mauiFile)
            if not okPath then
                bld.saveMsg = pathErr
            else
...
local function tstEval(code)
    ...
    if code:find('%$%{') then
        ...
        local unsafeToken = findUnsafeLuaToken(resolved)
        if unsafeToken then return nil,'BLOCKED: unsafe Lua token "'..unsafeToken..'"' end
        local fn,err = compileSandboxedExpression(resolved)
...
local function drawTester()
    ...
    ImGui.Spacing()
    if ImGui.Button((Icons.FA_PLAY_CIRCLE_O or Icons.FA_PLAY or '')..' Test Once',118,0) then tstRunOnce() end ImGui.SameLine()
    if ImGui.Button((Icons.FA_ARROW_LEFT or '')..' Builder',96,0)  then tst.input=bld.input end ImGui.SameLine()
    if ImGui.Button((Icons.FA_ARROW_RIGHT or '')..' Builder',98,0)  then bldLoad(tst.input) end ImGui.SameLine()
    if tst.running then
        if ImGui.Button((Icons.FA_STOP_CIRCLE_O or Icons.FA_STOP or '')..' Stop Timer',118,0) then tst.running=false end
    else
        if ImGui.Button((Icons.FA_PLAY_CIRCLE or Icons.FA_PLAY or '')..' Start Timer',122,0) then
    ...
    ImGui.SameLine()
    if ImGui.Button((Icons.FA_TRASH_O or Icons.FA_TIMES or '')..' Clear Log',108,0) then tst.log={} end
    if tst.running then
        ImGui.SameLine()
...
--  TAB 10 — EVENT HOOKS
-- ══════════════════════════════════════════════════════
local evts={hooks={},newName='',newPat='',newCond='',newAct='',log={},active=false,allowCommandExecution=false}
            ...
            local ok=pcall(function()
                mq.event(h.name,h.pattern,function()
                    ...
                    if fire and ref.action~='' then
                        local okCmd, cmdErr = isSafeHookCommand(ref.action)
                        if evts.allowCommandExecution and okCmd then
                            mq.cmd(ref.action)
                            logMsg(evts.log,'FIRED ['..ref.name..']: '..ref.action)
                        else
                            logMsg(evts.log,'BLOCKED ['..ref.name..']: '..(cmdErr or 'command execution disabled'))
                        end
                    else
...
local function drawEventHooks()
    ...
    local allowExec, allowExecChanged = ImGui.Checkbox('Allow hook command execution', evts.allowCommandExecution)
    if allowExecChanged then evts.allowCommandExecution = allowExec end
    if not evts.allowCommandExecution then
        ImGui.SameLine()
        ImGui.TextColored(C.WARN[1],C.WARN[2],C.WARN[3],C.WARN[4],'safe mode: matching hooks will log, not execute')
    end
    ImGui.Spacing()
