...
function M.setup(env)
  ...
  local exportInventoryJson = env.exportInventoryJson
  ...
  local helpInfo = {
    ...
    { binding = "/ezinventory_export [path]",   description = "Exports inventory snapshot JSON for external tools." },
  ...
  mq.bind("/ezinventory_toggle_detailed", function()
    ...
    mq.bind('/ezinventory_export', function(pathArg)
        if not exportInventoryJson then
            printf("[EZInventory] Export handler not available.")
            return
        end
        local ok, result = exportInventoryJson("manual", true, pathArg)
        if ok then
            printf("[EZInventory] Exported inventory JSON to: %s", tostring(result))
        else
            printf("[EZInventory] Export failed: %s", tostring(result))
        end
    end)
end
