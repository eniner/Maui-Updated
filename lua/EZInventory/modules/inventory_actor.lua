...
local get_detailed_item_stats

local function get_basic_item_info(item, include_extended_stats, include_detailed_stats)
    local basic = {}
    ...
    if include_extended_stats then
        ...
        basic.endurance = safe_get(function() return item.Endurance() end, 0)
        ...
        local detailed = get_detailed_item_stats(item, include_detailed_stats)
...
get_detailed_item_stats = function(item, force_detailed_stats)
    local stats = {}
    ...
    if not force_detailed_stats and not M.config.loadDetailedStats then
        return stats
    end
...
function M.gather_inventory(options)
    ...
    local includeDetailedStats = options.includeDetailedStats
    if includeDetailedStats == nil then
        includeDetailedStats = M.config.loadDetailedStats
    end
    includeDetailedStats = not not includeDetailedStats
    ...
    for slot = 0, 22 do
        local item = mq.TLO.Me.Inventory(slot)
        if item() then
            local entry = get_basic_item_info(item, includeExtendedStats, includeDetailedStats)
            entry.slotid = slot
    ...
    for invSlot = 23, 34 do
        local pack = mq.TLO.Me.Inventory(invSlot)
        if pack() then
            local generalEntry = get_basic_item_info(pack, includeExtendedStats, includeDetailedStats)
        ...
        if item.ID() then
            ...
            if item.Container() and item.Container() > 0 then
                for i = 1, item.Container() do
                    local sub = item.Item(i)
                    if sub.ID() then
                        local subEntry = get_basic_item_info(sub, includeExtendedStats, includeDetailedStats)
