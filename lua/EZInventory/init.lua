...
local DefaultExportFile   = string.format('%s/EZInventory/%s/%s_inventory.json', mq.configDir, server, mq.TLO.Me.CleanName())
local Settings            = {}
local searchText          = ""
local lastExportFingerprint = nil
...
local function normalizePath(rawPath)
    return tostring(rawPath or ""):gsub("^%s+", ""):gsub("%s+$", "")
end

local function getExportFilePath(pathOverride)
    local chosen = normalizePath(pathOverride)
    if chosen ~= "" then
        return chosen
    end

    local configured = normalizePath(Settings.jsonExportFile)
    if configured ~= "" then
        return configured
    end
...
local function buildExportPayload(data, reason)
    return {
        exportedAt = os.date("!%Y-%m-%dT%H:%M:%SZ"),
        exportedBy = tostring(mq.TLO.Me.CleanName() or ""),
...
local function computeExportFingerprint(data)
...
local function exportInventoryJson(reason, force, pathOverride)
    if not inventory_actor then
        return false, "inventory actor unavailable"
    end

    local exportPath = getExportFilePath(pathOverride)
    if exportPath == "" then
        return false, "no export file path configured"
    end

    local data = (inventory_actor.get_cached_inventory and inventory_actor.get_cached_inventory(true)) or nil
    local needsEnriched = true
    if data and data.equipped and #data.equipped > 0 then
        local sample = data.equipped[1]
        if sample and (sample.str ~= nil or sample.heroicStr ~= nil) then
            needsEnriched = false
        end
    end
    if not data or needsEnriched then
    ...
    if not data then
        data = inventory_actor.gather_inventory({ includeExtendedStats = false, scanStage = "fast" })
    end
    if not data then
        return false, "no inventory data available"
    end

    local fingerprint = computeExportFingerprint(data)
    if not force and fingerprint == lastExportFingerprint and exportPath == lastExportPath then
        return true, "unchanged"
    end

    local payload = buildExportPayload(data, reason)
    local encoded, err = json.encode(payload, { indent = true })
    if not encoded then
        return false, tostring(err or "json encode failed")
    end

    local file, fileErr = io.open(exportPath, "w")
    if not file then
        return false, string.format("cannot write %s (%s)", tostring(exportPath), tostring(fileErr or "unknown error"))
    end
    file:write(encoded)
    file:write("\n")
    file:close()
...
function inventoryUI.render()
    ...
    if open and show then
        ...
        local function renderHeaderCloseButton()
            local styleCount = Theme.push_button_variant(ImGui, "success")
            ...
            Theme.pop_button_variant(ImGui, styleCount)
        end
        ...
        local function renderHeaderViewModeButton()
            ...
            local styleCount = Theme.push_button_variant(ImGui, "primary")
        ...
        local function renderCleanupButton(cleanupButtonWidth)
            ...
            if cleanupInProgress then
                local styleCount = Theme.push_button_variant(ImGui, "warning")
...
Bindings.setup({
    ...
    exportInventoryJson = exportInventoryJson,
})

local function main()
    Bindings.displayHelp()
    print(string.format("[EZInventory] JSON export target: %s", getExportFilePath(nil)))
    ...
    while true do
        ...
        if Settings.autoExportJson then
            local interval = tonumber(Settings.autoExportSeconds) or 20
            if interval < 5 then interval = 5 end
            local now = os.time()
            inventoryUI._lastAutoExportTime = inventoryUI._lastAutoExportTime or 0
            if (now - inventoryUI._lastAutoExportTime) >= interval then
                local ok, result = exportInventoryJson("auto", false, nil)
                if ok and result ~= "unchanged" then
                    print(string.format("[EZInventory] Auto-exported inventory JSON: %s", tostring(result)))
                elseif not ok then
                    print(string.format("[EZInventory] Auto-export failed: %s", tostring(result)))
                end
                inventoryUI._lastAutoExportTime = now
            end
        end
